...
94. **Make Shrine and Event nodes live.** Promote the two reserved node types with their code-native decision surfaces, seed them into the generator (respecting the existing adjacency rules), and light up the first relic set (6) and first artifact (1) from the content catalogue as their reward pool. Art briefs for the Shrine/Event medallions and relic/artifact tiles are in `last-bastion-content.md`.
    ...
    **Remaining in-run gate (now three concrete steps):** (1) a run-state carrier — add `relicIds`/`equippedArtifactId` to `ExpeditionBuildSnapshot` and a `maxHealthBonus`/weapon-slot-grant carrier, persisted via an additive save-schema bump (the schema-v8 `transformation` field is the precedent); (2) combat consumption — `CombatSimulation` resolves `resolveRelicModifiers` at run start and reads the bag at the points it already reads `perkModifiers` (start simple: spread, explosive-damage, evasive distance), plus applying the carried max-health/slot grants; (3) the in-run event scene — route shrine/event selection from `ExpeditionScene` to a resolution screen (reuse `EncounterEventScene` logic) that reads the run build, resolves, routes `ambush` to combat, persists the carrier, and commits the node. Only after (1)–(3) does the generator budget promote shrine (1) and event (2).
