...
| Perks | Seven pre-drop modifiers live with milestone unlocks, persistence, selection, and map/combat integration | Complete through Task 46 |
...
46. **Perk system behavior gate. Completed 19 July 2026.** Character Select resolves one persisted pre-drop perk from seven canonical definitions. Records milestones unlock the pool; Veteran, Scrapper, Quartermaster, Fast Learner, Gunsmith, Survivor, and Pathfinder modify only their owned run/map boundaries. Save schema v3 adds nodes-cleared progress and selected perk persistence. Batch I tiles remain the shared presentation family.
...
53. **Fire-control accessibility gate.** Persist Auto-fire / Manual in `LocalSaveStore`, default Auto-fire, add the `T` / pad-R3 in-run toggle, keep aim independent, and define which cadence/support weapons intentionally ignore trigger mode. No raster art required; the HUD mode glyph is code-owned.
54. **Destructible-terrain health gate.** Replace the current two-hit boss-only obstacle state with numeric durability, damage events, conditional mini HP bars, projectile/melee/mini-boss damage rules, and route-preservation tests. Add fence, rock/boulder, and reinforced-cover definitions before Batch R art.
55. **Combat silhouette and player-hit feedback gate.** Add the larger outlined negative player-damage number, reduced-flash fallback, and an A/B lab for thin actor rims plus damage-family projectile halos across all five world themes. Promote outlines only if they improve grayscale crowd readability without muddying pixel silhouettes.
56. **Mini-boss mobility/threat pass.** Review Siege Crusher, Brood Warden, and Rift Stalker at 1.25–1.45× elite silhouette scale; add purposeful repositioning between attacks and more arena traversal while preserving locked tells, dodge routes, and punishable recoveries. Production sheets are extended only where existing locomotion frames fail at the approved scale.

Sequencing note: Tasks 44–47 are complete. **Next recommended implementation: Task 48, Node-budget encounter director.** Task 49 remains the final consolidated tuning pass before the Web MVP gate. The Corrupted Human family (Batch M) and sector campaign stay deferred until the single-sector MVP succeeds.
