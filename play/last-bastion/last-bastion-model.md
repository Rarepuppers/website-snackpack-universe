...
61. **Steam readiness gate. In progress 22 July 2026.** Keep simulation independent from Phaser presentation behind a desktop-sized viewport contract. The first platform-neutral slice is live: replay format v1 records a safe-integer seed, simulation-compatibility version, canonical 1/60-second timestep, and compressed input spans; playback rejects unsupported formats/rules/timesteps and produces a stable state digest guarded by a committed fixture. **Remaining:** broader replay coverage across decisions/expedition nodes, offline boot/service-worker audit, achievement event contract, cloud-save conflict policy, and a Steam Input/SDK integration spike before platform-specific production work.
...
Sequencing note: Tasks 44–50 and 53–60 are complete. **Next: Task 61 Steam foundations.** Task 62 owns Corrupted Human normal-wave promotion and may advance as a self-contained gameplay-content track before or alongside the platform work. Recommended content order after that gate is **Task 63 Nest Weaver → Task 64 Storm Savant → Task 65 Machine uprising**, because each introduces one reusable summoning/chain/support rule before Task 66 combines them at mini-boss scale. Keep the sector campaign deferred until the single-sector MVP succeeds.
