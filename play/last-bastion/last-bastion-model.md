...
60. **Accessibility and display gate. In progress 20 July 2026.** A simulation-space title-safe rectangle now anchors the major HUD edges and projects exactly at 960×540, Full HD, and 4K. Reduced flash remains persisted; focus/visibility loss pauses without auto-resuming; every major danger family has a unique non-colour shape signature; and floating combat text is capped at 24 labels while protecting player-damage/heal feedback from ordinary-number recycling. **Remaining:** persisted per-action keyboard/controller remapping, binding-aware HUD/help copy, and a creator controller/colour-vision review before completion.
61. **Steam readiness gate.** Keep simulation independent from Phaser presentation behind a desktop-sized viewport contract; add fixed-step replay fixtures, seed/version compatibility, save migration, offline boot, and a Steam Input/achievement/cloud-save integration spike before platform-specific production work.
62. **Corrupted Human family completion gate.** Implement the Abomination's shamble → slam/grab tells → committed impact → vulnerable recovery, add off-screen warning coverage for ranged Marines, then tune mixed Survivor/Marine/Abomination pulses before any normal Quick Drop or expedition promotion. Keep Abomination Prime deferred.
63. **Nest Weaver alien-summoner gate.** Add a mobile alien breeder that places destructible egg pods behind a locked placement tell. Each pod exposes a visible hatch clock and dies before hatching if focused; surviving pods release a small authored payload. Enforce a hard two-pod live cap, no chain-spawning offspring, and charge every pod/hatch against the encounter's existing threat and live-unit budgets. The Weaver must enter a long exposed recovery after laying. Reuse Egg Cluster lifecycle rules where they fit, but keep this unit tactically distinct from the static Egg Cluster and Brood Warden.
64. **Storm Savant science-lightning gate.** Add a rogue xenotechnician whose coil/psionic apparatus reads like battlefield magic while remaining grounded in the science-fantasy setting. Its primary attack marks conductive nodes, then draws fixed chain-lightning segments after a long shape-and-audio tell; moving out of a marked segment or destroying an exposed node breaks the chain. Cap hops, forbid retargeting after lock, preserve at least one escape lane, and give the Savant a vulnerable overload recovery. Production art follows deterministic hit, cover, chain, and colour-independent telegraph tests.
65. **Machine uprising family gate.** Prototype Shock-weak android/cyborg enemies in this order: Scrap Skitterer common, Arc Warden ranged specialist, Cyborg Reclaimer support bruiser, then the **Foundry Fabricator** summoner. Fabrication is an interruptible channel with finite charges and a visible output pad; spawned drones/turrets consume live slots and threat already reserved by the Fabricator, have no recursive summon path, and self-expire if their owner exits. Only after those roles read cleanly together may the Assembly Prime/Foundry Mind mini-boss behavior gate begin.
66. **Mini-boss roster expansion.** Grow beyond the initial three only from accepted standard/elite families. Candidate order: Synapse Herald (telegraph mastery), Assembly Prime (machine fabrication and lane control), Storm Regent (lightning-node arena puzzle), then Abomination Prime. Every candidate needs two to four distinct moves, a 45–90 second target, seeded no-immediate-repeat selection, a guaranteed reward, off-screen warnings, a punishable recovery, and a behavior lab before production art or pool promotion.

Sequencing note: Tasks 44–50 and 53–59 are complete. **Next: Task 60, accessibility/display, with Task 61 Steam foundations after its viewport/input contracts settle.** Task 62 owns Corrupted Human normal-wave promotion and may advance as a self-contained content track. Recommended content order after that gate is **Task 63 Nest Weaver → Task 64 Storm Savant → Task 65 Machine uprising**, because each introduces one reusable summoning/chain/support rule before Task 66 combines them at mini-boss scale. Keep the sector campaign deferred until the single-sector MVP succeeds.
