...
**Current objective:** the mechanical half of Task 33 is done (61/61 review routes swept clean via `dev/review-harness.html`), and Tasks 36–38 are implemented as of 19 July 2026 — the creator sitting now owes visual/feel judgement across the queued items plus the shell and map screens. Next code work: **Task 39, node → encounter wiring** (map nodes drive the arena with budgets by type and depth, build carried through the schema-v2 snapshot), then Tasks 44–52. Event Horizon remains held until its own behavior gate; Batches G1/G2/O art wait on their creator reviews.
