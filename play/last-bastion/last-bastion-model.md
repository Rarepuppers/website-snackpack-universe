...
43. **Status-effect presentation gate.** Floating damage numbers with the shared colour language (ivory physical, red fire, teal shock, blue frozen, green toxic) and Settings toggle, healing ticks, and code-native status overlays (tint plus placeholder particles) for Blaze/Overload/Freeze/Toxic — then Production Asset Batch K (briefs in `last-bastion-content.md`) replaces the placeholders. Lands naturally inside step 35's pacing/rescale work.
