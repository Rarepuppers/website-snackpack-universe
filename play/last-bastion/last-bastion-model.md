...
**Current objective:** continue step 35 with per-wave scaling and the timed threat-budget director now that the 2-damage / 10-health combat rescale is implemented; then add the shared fractional-projectile accumulator. The consolidated creator playtest remains the tuning gate before Event Horizon production promotion.
