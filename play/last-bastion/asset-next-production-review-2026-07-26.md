...
- Character-select audit: Marine, Medic, and Assault are production-playable hero definitions with complete gameplay sheets and full-height select portraits. Assault is earned through Armory clearance. Tactician has a test-covered mechanics candidate plus retained select portrait, gameplay sheet, and roster-tile candidates behind explicit C3 review routes; ordinary Character Select deliberately retains its silhouette and disabled Deploy action until audio, tuning, unlock economy, complete visual review, and release acceptance pass. Scout remains a placeholder.
...
50. **Character-select showcase batch:** Marine and Medic have paired full-height, text-free select portraits under `art/production-tests/batch-character-select/`; Assault's accepted C3 portrait is wired beside them. Add Tactician/Scout only after their mechanics are real; reserve Alien/Cultist/Cyborg for a later secret-roster batch. Portraits are retained at 1024x1536 for Steam close-view use, with gameplay sheets remaining separate.
...
- **Future heroes still need presentation.** Assault now has its complete C3 presentation; Tactician and Scout remain bare silhouette placeholders, and Alien/Cultist/Cyborg are reserved with nothing to show. A locked slot still needs to look deliberate.
...
66. **Character Batch C2 — locked, mystery, and reveal plates.** Every roster slot must look intentional, including the ones you cannot pick yet. Three presentation states, at the same 1024x1536 full-height format as the accepted Marine/Medic select portraits:
    - **Locked** — a hero whose identity is known but not yet available (Tactician, Scout). Use a readable, deliberately unlit silhouette rather than a blank. Assault has superseded its locked plate with the accepted C3 package and Armory requirement state.
