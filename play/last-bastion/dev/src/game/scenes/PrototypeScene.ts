...
export class PrototypeScene extends Phaser.Scene {
  ...
  private eventPresenter!: CombatEventPresenter;
  ...
  create(): void {
    ...
    const presentationScene = this;
    this.eventPresenter = new CombatEventPresenter({
      haptics: this.haptics,
      synth: this.synth,
      damageNumbers: this.damageNumbers,
      effectPool: this.effectPool,
      eventFeed: this.eventFeed,
      get settings() { return presentationScene.settings; },
      get lastSnapshot() { return presentationScene.lastSnapshot; },
      get time() { return presentationScene.time; },
      pulseWeapon: (instanceId) => this.pulseWeapon(instanceId),
      animateProductionWeapon: (instanceId, weaponId) => this.animateProductionWeapon(instanceId, weaponId),
      animatePatrolBlade: (instanceId) => this.animatePatrolBlade(instanceId),
      drawPatrolBladeSweep: (position, direction, weaponId) => this.drawPatrolBladeSweep(position, direction, weaponId),
      drawChainArc: (from, to) => this.drawChainArc(from, to),
      emitAuthoredEffect: (...args) => this.emitAuthoredEffect(...args),
      emitNestEffect: (...args) => this.emitNestEffect(...args),
      emitSkittererEffect: (...args) => this.emitSkittererEffect(...args),
      emitStormEffect: (...args) => this.emitStormEffect(...args),
      emitArcWardenEffect: (...args) => this.emitArcWardenEffect(...args),
      emitReclaimerEffect: (...args) => this.emitReclaimerEffect(...args),
      emitFoundryEffect: (...args) => this.emitFoundryEffect(...args),
      emitSynapseHeraldEffect: (...args) => this.emitSynapseHeraldEffect(...args),
      emitAssemblyPrimeEffect: (...args) => this.emitAssemblyPrimeEffect(...args),
      emitStormRegentEffect: (...args) => this.emitStormRegentEffect(...args),
      emitAbominationPrimeEffect: (...args) => this.emitAbominationPrimeEffect(...args),
      flashCircle: (...args) => this.flashCircle(...args),
      shakeCamera: (duration, intensity) => this.shakeCamera(duration, intensity),
      flashCamera: (duration, red, green, blue) => this.flashCamera(duration, red, green, blue),
      showDamageDirection: () => this.showDamageDirection(),
      showPickupBanner: (type) => this.showPickupBanner(type),
      usesBladeBody,
      powerupDisplayName,
    });
  ...
  update(_time: number, deltaMilliseconds: number): void {
    ...
    if (plan.steps > 0) {
      this.eventPresenter.play(snapshot.events);
