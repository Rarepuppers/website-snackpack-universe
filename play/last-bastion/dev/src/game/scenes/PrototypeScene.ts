...
export class PrototypeScene extends Phaser.Scene {
  ...
  private readonly nestTelegraphs = new Map<number, Phaser.GameObjects.Graphics>();
  private readonly stormTelegraphs = new Map<number, Phaser.GameObjects.Graphics>();
  private readonly scrapSkittererTelegraphs = new Map<number, Phaser.GameObjects.Graphics>();
  private readonly arcWardenTelegraphs = new Map<number, Phaser.GameObjects.Graphics>();
  private readonly reclaimerLinks = new Map<number, Phaser.GameObjects.Graphics>();
  private readonly foundryTurretTelegraphs = new Map<number, Phaser.GameObjects.Graphics>();
  ...
  private renderSnapshot(snapshot: CombatSnapshot, firing: boolean): void {
    ...
    this.syncNestTelegraphs(snapshot.enemies);
    this.syncStormTelegraphs(snapshot.enemies);
    this.syncScrapSkittererTelegraphs(snapshot.enemies);
    this.syncArcWardenTelegraphs(snapshot.enemies);
    this.syncReclaimerLinks(snapshot.enemies);
    this.syncFoundryTurretTelegraphs(snapshot.enemies);
  ...
  private restartRun(): void {
    ...
    for (const views of [
      ...
      this.scrapSkittererTelegraphs,
      this.arcWardenTelegraphs,
      this.reclaimerLinks,
      this.foundryTurretTelegraphs,
      ...
      switch (event.type) {
        ...
        case "enemy-defeated":
          if (event.enemyType === "aurum-hoarder") {
            ...
            this.emitAssemblyPrimeEffect(3, event.position, 620, 0.92, 2.3);
          } else if (event.enemyType === "abomination-prime") {
            this.emitAbominationPrimeEffect(3, event.position, 680, 0.96, 2.45);
        ...
        case "enemy-spawned":
          if (event.enemyType === "ripper") {
            ...
            this.emitStormEffect(0, event.position, 220, 0.42, 0.84);
          } else {
        ...
        case "storm-chain-warning":
          this.emitStormEffect(1, event.position, 240, 0.46, 0.9);
          break;
        case "storm-chain-discharged":
          this.emitStormEffect(2, event.position, 180, 0.52, 1.02);
          break;
        case "storm-chain-interrupted":
          this.emitStormEffect(3, event.position, 320, 0.48, 0.96);
          break;
        case "scrap-skitterer-warning":
          this.emitSkittererEffect(0, event.position, 190, 0.4, 0.78, Math.atan2(event.direction.y, event.direction.x));
          break;
        case "scrap-skitterer-rush":
          this.emitSkittererEffect(1, event.position, 180, 0.44, 0.9, Math.atan2(event.direction.y, event.direction.x));
          break;
        case "scrap-skitterer-impact":
          this.emitSkittererEffect(2, event.position, 220, 0.52, 0.98);
          break;
        case "scrap-skitterer-wreck":
          this.emitSkittererEffect(3, event.position, 360, 0.42, 0.84);
          break;
        case "arc-warden-warning":
          this.emitArcWardenEffect(0, event.position, 240, 0.45, 0.9);
          break;
        case "arc-warden-discharged":
          this.emitArcWardenEffect(1, event.position, 170, 0.5, 1);
          this.emitArcWardenEffect(2, event.endpoint, 210, 0.48, 0.94);
          this.time.delayedCall(190, () => this.emitArcWardenEffect(3, event.position, 300, 0.44, 0.88));
          break;
        case "reclaimer-link-started":
          this.emitReclaimerEffect(0, event.position, 240, 0.48, 0.9);
          break;
        case "reclaimer-repair-completed":
          this.emitReclaimerEffect(1, event.target, 300, 0.55, 1.08);
          this.time.delayedCall(180, () => this.emitReclaimerEffect(3, event.position, 320, 0.46, 0.9));
          break;
        case "reclaimer-link-interrupted":
          this.emitReclaimerEffect(2, event.position, 260, 0.52, 1.02);
          this.time.delayedCall(150, () => this.emitReclaimerEffect(3, event.position, 320, 0.44, 0.86));
          break;
        case "foundry-fabrication-started":
          this.emitFoundryEffect(0, event.position, 280, 0.5, 0.96);
          break;
        case "foundry-fabrication-completed":
          this.emitFoundryEffect(1, event.position, 340, 0.62, 1.18);
          break;
        case "foundry-fabrication-interrupted":
          this.emitFoundryEffect(2, event.position, 300, 0.58, 1.12);
          break;
        case "foundry-turret-fired":
          this.emitFoundryEffect(3, event.position, 190, 0.52, 0.96, Math.atan2(event.target.y - event.position.y, event.target.x - event.position.x));
          break;
        case "synapse-herald-warning":
          if (event.move === "lunge-chain") {
            this.emitSynapseHeraldEffect(0, event.position, 260, 0.5, 0.98);
          }
          break;
        case "synapse-herald-lunge":
          this.emitSynapseHeraldEffect(0, event.position, 190, 0.52, 1.04);
          break;
        case "synapse-herald-zones-erupted":
          for (const zone of event.zones) this.emitSynapseHeraldEffect(1, zone, 320, 0.62, 1.2);
          break;
        case "synapse-herald-link-started":
          this.emitSynapseHeraldEffect(2, event.position, 280, 0.55, 1.08);
          break;
        case "synapse-herald-link-broken":
          this.emitSynapseHeraldEffect(3, event.position, 300, 0.58, 1.14);
          break;
        case "assembly-prime-warning":
          if (event.move === "rotating-lanes") this.emitAssemblyPrimeEffect(0, event.position, 300, 0.56, 1.08);
          break;
        case "assembly-prime-fabrication-completed":
          this.emitAssemblyPrimeEffect(1, event.position, 360, 0.64, 1.24);
          break;
        case "assembly-prime-fabrication-interrupted":
          this.emitFoundryEffect(2, event.position, 300, 0.58, 1.12);
          break;
        case "assembly-prime-drone-recalled":
          this.emitAssemblyPrimeEffect(2, event.position, 320, 0.6, 1.18);
          break;
        case "storm-regent-warning":
          if (event.move === "chain-strike") this.emitStormRegentEffect(0, event.position, 280, 0.54, 1.06);
          break;
        case "storm-regent-discharged":
          if (event.move === "chain-strike") {
            this.emitStormRegentEffect(0, event.position, 220, 0.58, 1.14);
          } else if (event.move === "node-overcharge") {
            this.emitStormRegentEffect(1, event.centre ?? event.position, 340, 0.68, 1.28);
          } else {
            this.emitStormRegentEffect(2, event.centre ?? event.position, 300, 0.64, 1.22);
          }
          break;
        case "storm-regent-interrupted":
          this.emitStormRegentEffect(3, event.nodePosition ?? event.position, 320, 0.6, 1.18);
          break;
        case "abomination-prime-slam":
          this.emitAbominationPrimeEffect(0, event.position, 380, 0.72, 1.42);
          if (event.hitPlayer) this.shakeCamera(150, 0.006);
          break;
        case "abomination-prime-grab-latched":
          this.emitAbominationPrimeEffect(1, event.position, 280, 0.62, 1.16);
          break;
        case "abomination-prime-grab-broken":
          this.emitAbominationPrimeEffect(1, event.position, 320, 0.58, 1.22);
          break;
        case "abomination-prime-biomass-thrown":
          this.emitAbominationPrimeEffect(2, event.position, 300, 0.64, 1.18);
          break;
        case "abomination-prime-biomass-landed":
          this.emitAuthoredEffect(3, event.position, 300, 0.62, 1.04, 0, "abomination-prime-biomass-v1");
          break;
  ...
  private emitSkittererEffect(
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, rotation, "machine-scrap-skitterer-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, rotation, "machine-scrap-skitterer-effects-v1");
    });
  }

  private emitStormEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, 0, "storm-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, 0, "storm-effects-v1");
    });
  }

  private emitArcWardenEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, 0, "machine-arc-warden-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, 0, "machine-arc-warden-effects-v1");
    });
  }

  private emitReclaimerEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, 0, "machine-cyborg-reclaimer-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, 0, "machine-cyborg-reclaimer-effects-v1");
    });
  }

  private emitFoundryEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, rotation, "machine-foundry-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, rotation, "machine-foundry-effects-v1");
    });
  }

  private emitSynapseHeraldEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, 0, "synapse-herald-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, 0, "synapse-herald-effects-v1");
    });
  }

  private emitAssemblyPrimeEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, 0, "assembly-prime-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, 0, "assembly-prime-effects-v1");
    });
  }

  private emitStormRegentEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, 0, "storm-regent-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, 0, "storm-regent-effects-v1");
    });
  }

  private emitAbominationPrimeEffect(
    column: 0 | 1 | 2 | 3,
    position: { x: number; y: number },
    duration: number,
    scale: number,
    ...
    this.emitAuthoredEffect(column, position, duration, scale, targetScale, 0, "abomination-prime-effects-v1");
    this.time.delayedCall(Math.round(duration * 0.48), () => {
      this.emitAuthoredEffect(column + 4, position, Math.round(duration * 0.68), scale * 0.9, targetScale * 1.04, 0, "abomination-prime-effects-v1");
    });
  }
  ...
  private createEnemyView(enemy: EnemySnapshot): EnemyView {
    ...
    switch (enemy.type) {
      ...
      case "nest-hatchling":
        ...
        return this.add.triangle(0, 0, 18, 0, -12, -9, -12, 9, 0x6e54a3)
          .setStrokeStyle(2, 0x7fe7d9);
      case "storm-savant":
        if (this.useMarineArt) {
          return createManifestSprite(this, "storm-savant-v1");
        }
        return this.add.triangle(0, 0, -23, -20, 28, 0, -23, 20, 0x34536f)
          .setStrokeStyle(4, 0x8de7ff);
      case "storm-node":
        if (this.useMarineArt) {
          return createManifestSprite(this, "storm-node-v1");
        }
        return this.add.circle(0, 0, 14, 0x263b51).setStrokeStyle(4, 0xffe47a);
      case "scrap-skitterer":
        if (this.useMarineArt) {
          return createManifestSprite(this, "machine-scrap-skitterer-v1");
        }
        return this.add.triangle(0, 0, 20, 0, -14, -11, -14, 11, 0x59636b)
          .setStrokeStyle(3, 0xffd36b);
      case "arc-warden":
        if (this.useMarineArt) {
          return createManifestSprite(this, "machine-arc-warden-v1");
        }
        return this.add.rectangle(0, 0, 30, 42, 0x344754)
          .setStrokeStyle(4, 0x8de7ff);
      case "cyborg-reclaimer":
        if (this.useMarineArt) {
          return createManifestSprite(this, "machine-cyborg-reclaimer-v1");
        }
        return this.add.ellipse(0, 0, 40, 48, 0x42505a)
          .setStrokeStyle(5, 0x8ff0ae);
      case "foundry-fabricator":
        if (this.useMarineArt) return createManifestSprite(this, "machine-foundry-fabricator-v1");
        return this.add.rectangle(0, 0, 46, 50, 0x3f4650)
          .setStrokeStyle(5, 0xffb85c);
      case "foundry-pad":
        if (this.useMarineArt) return createManifestSprite(this, "machine-foundry-pad-v1");
        return this.add.ellipse(0, 0, 44, 28, 0x4a3828)
          .setStrokeStyle(5, 0xffd36b);
      case "foundry-drone":
        if (this.useMarineArt) return createManifestSprite(this, "machine-foundry-drone-v1");
        return this.add.triangle(0, 0, 18, 0, -13, -10, -13, 10, 0x68727c)
          .setStrokeStyle(3, 0xffb85c);
      case "foundry-turret":
        if (this.useMarineArt) return createManifestSprite(this, "machine-foundry-turret-v1");
        return this.add.rectangle(0, 0, 30, 30, 0x4b5661)
          .setStrokeStyle(4, 0x8de7ff);
      case "blast-mite":
        if (this.useMarineArt) {
      ...
      case "synapse-herald":
        if (this.useMarineArt) {
          return createManifestSprite(this, "synapse-herald-v1");
        }
        return this.add.ellipse(0, 0, 74, 58, 0x49345f).setStrokeStyle(6, 0xe5a4ff);
      case "assembly-prime":
        if (this.useMarineArt) return createManifestSprite(this, "assembly-prime-v1");
        return this.add.rectangle(0, 0, 82, 68, 0x35485f).setStrokeStyle(6, 0xffb84d);
      case "storm-regent":
        if (this.useMarineArt) return createManifestSprite(this, "storm-regent-v1");
        return this.add.ellipse(0, 0, 80, 66, 0x263b54).setStrokeStyle(6, 0x8de7ff);
      case "abomination-prime":
        if (this.useMarineArt) return createManifestSprite(this, "abomination-prime-v1");
  ...
  private styleEnemyView(view: EnemyView, enemy: EnemySnapshot): void {
    if (view instanceof Phaser.GameObjects.Sprite) {
      const batchJScale = view.texture.key === "abomination-prime-v1" ? 0.72
        : view.texture.key === "synapse-herald-v1" || view.texture.key === "assembly-prime-v1" || view.texture.key === "storm-regent-v1" ? 0.62
        : view.texture.key === "quillback-matriarch-v1" ? 0.9
    ...
    if (enemy.type === "storm-savant" && view instanceof Phaser.GameObjects.Triangle) {
      const phase = enemy.stormPhase ?? "idle";
      const colors = {
        idle: 0x34536f,
        tell: 0x477ca1,
        discharge: 0xeafcff,
        "overload-recovery": 0x50545f,
      } as const;
      view.setFillStyle(colors[phase])
        .setStrokeStyle(4, phase === "tell" ? 0xffe47a : phase === "discharge" ? 0xffffff : 0x8de7ff)
        .setRotation(Math.atan2(enemy.facingDirection.y, enemy.facingDirection.x))
        .setScale(phase === "tell" ? 1.12 : phase === "overload-recovery" ? 0.88 : 1)
        .setAlpha(phase === "overload-recovery" ? 0.75 : 1);
      return;
    }
    if (enemy.type === "storm-node" && view instanceof Phaser.GameObjects.Arc) {
      const pulse = 1 + Math.sin(this.time.now / 90) * 0.09;
      view.setScale(pulse).setAlpha(0.9);
      return;
    }
    if (enemy.type === "scrap-skitterer" && view instanceof Phaser.GameObjects.Triangle) {
      const phase = enemy.scrapSkittererPhase ?? "approach";
      const colors = {
        approach: 0x59636b,
        "rush-windup": 0xc4973e,
        rush: 0xe9d36a,
        brake: 0x434950,
      } as const;
      const direction = phase === "approach"
        ? enemy.facingDirection
        : enemy.scrapSkittererDirection ?? enemy.facingDirection;
      view.setFillStyle(colors[phase])
        .setStrokeStyle(3, phase === "rush-windup" ? 0xffffff : 0xffd36b)
        .setRotation(Math.atan2(direction.y, direction.x))
        .setScale(phase === "rush-windup" ? 1.16 : phase === "brake" ? 0.86 : 1)
        .setAlpha(phase === "brake" ? 0.72 : 1);
      return;
    }
    if (enemy.type === "arc-warden" && view instanceof Phaser.GameObjects.Rectangle) {
      ...
      view.setFillStyle(colors[phase])
        .setStrokeStyle(4, phase === "charge" ? 0xffe47a : phase === "discharge" ? 0xffffff : 0x8de7ff)
        .setRotation(Math.atan2(enemy.facingDirection.y, enemy.facingDirection.x) + Math.PI / 2)
        .setScale(phase === "charge" ? 1.1 : phase === "recovery" ? 0.9 : 1)
        .setAlpha(phase === "recovery" ? 0.72 : 1);
      return;
    }
    if (enemy.type === "cyborg-reclaimer" && view instanceof Phaser.GameObjects.Ellipse) {
      const phase = enemy.reclaimerPhase ?? "seeking";
      const colors = { seeking: 0x42505a, channel: 0x3f775c, recovery: 0x55505b } as const;
      view.setFillStyle(colors[phase])
        .setStrokeStyle(5, phase === "channel" ? 0xffe47a : phase === "recovery" ? 0xc18fb4 : 0x8ff0ae)
        .setRotation(Math.atan2(enemy.facingDirection.y, enemy.facingDirection.x) + Math.PI / 2)
        .setScale(phase === "channel" ? 1.08 : phase === "recovery" ? 0.9 : 1)
        .setAlpha(phase === "recovery" ? 0.75 : 1);
      return;
    }
    if (enemy.type === "foundry-fabricator" && view instanceof Phaser.GameObjects.Rectangle) {
      const phase = enemy.foundryPhase ?? "positioning";
      view.setFillStyle(phase === "channel" ? 0x6e512e : phase === "recovery" ? 0x4b4650 : 0x3f4650)
        .setStrokeStyle(5, phase === "channel" ? 0xffe47a : phase === "recovery" ? 0xa98da7 : 0xffb85c)
        .setRotation(Math.atan2(enemy.facingDirection.y, enemy.facingDirection.x) + Math.PI / 2)
        .setScale(phase === "channel" ? 1.08 : phase === "recovery" ? 0.92 : 1)
        .setAlpha(phase === "recovery" ? 0.76 : 1);
      return;
    }
    if (enemy.type === "foundry-pad" && view instanceof Phaser.GameObjects.Ellipse) {
      const pulse = 1 + Math.sin(this.time.now / 90) * 0.08;
      view.setFillStyle(0x5b4328)
        .setStrokeStyle(4, Math.floor(this.time.now / 160) % 2 === 0 ? 0xffe47a : 0xff9f43)
        .setScale(pulse)
        .setAlpha(0.94);
      return;
    }
    if (enemy.type === "foundry-drone" && view instanceof Phaser.GameObjects.Triangle) {
      view.setRotation(Math.atan2(enemy.facingDirection.y, enemy.facingDirection.x))
        .setAlpha(0.9 + Math.sin(this.time.now / 75) * 0.1);
      return;
    }
    if (enemy.type === "foundry-turret" && view instanceof Phaser.GameObjects.Rectangle) {
      const phase = enemy.foundryTurretPhase ?? "tracking";
      view.setRotation(Math.atan2(enemy.facingDirection.y, enemy.facingDirection.x) + Math.PI / 4)
        .setFillStyle(phase === "warning" ? 0x665d42 : phase === "recovery" ? 0x444a51 : 0x4b5661)
        .setStrokeStyle(4, phase === "warning" ? 0xffe47a : phase === "recovery" ? 0x8a93a0 : 0x8de7ff)
        .setScale(phase === "warning" ? 1.08 : phase === "recovery" ? 0.92 : 1 + Math.sin(this.time.now / 120) * 0.04);
      return;
    }
    ...
    if (enemy.type === "egg-cluster" || enemy.type === "nest-pod") {
    ...
    if (enemy.type === "synapse-herald" && view instanceof Phaser.GameObjects.Ellipse) {
      const phase = enemy.synapseHeraldPhase ?? "entrance";
      const move = enemy.synapseHeraldMove;
      const fill = phase === "windup" ? 0x6d4780
        : phase === "action" ? 0x8a4e8f
          : phase === "recovery" ? 0x4a4658 : 0x49345f;
      view.setFillStyle(fill)
        .setStrokeStyle(6, move === "synapse-link" ? 0x8de7ff : phase === "windup" ? 0xffe47a : 0xe5a4ff)
        .setRotation(Math.atan2(enemy.facingDirection.y, enemy.facingDirection.x))
        .setScale(phase === "windup" ? healthScale * 1.1 : phase === "recovery" ? healthScale * 0.9 : healthScale)
        .setAlpha(phase === "recovery" ? 0.78 : 1);
    }
  }
  ...
  private updateEnemySprite(
    ...
    switch (enemy.type) {
      ...
      case "storm-savant": {
        const canonicalColumn = cardinalFacingColumn(enemy.position, {
          x: enemy.position.x + enemy.facingDirection.x,
          y: enemy.position.y + enemy.facingDirection.y,
        });
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.stormPhase ?? "idle";
        const row = phase === "tell" ? 4 : phase === "discharge" ? 5 : phase === "overload-recovery" ? 6 : 1 + ((Math.floor(this.time.now / 180) + enemy.id) % 2);
        view.setTexture("storm-savant-v1").setFrame(row * 4 + facingColumn).setRotation(0).setScale(0.58);
        return;
      }
      case "storm-node": {
        const owner = this.lastSnapshot.enemies.find((candidate) => candidate.id === enemy.stormNodeOwnerId);
        if (owner?.type === "storm-regent") {
          const phase = owner.stormRegentPhase ?? "entrance";
          const committed = phase === "windup" || phase === "action";
          const isRelay = (owner.stormRegentSegments ?? []).some((segment) => (
            Math.hypot(segment.to.x - enemy.position.x, segment.to.y - enemy.position.y) < 0.08
            || Math.hypot(segment.from.x - enemy.position.x, segment.from.y - enemy.position.y) < 0.08
          ));
          const frame = enemy.health <= 0 ? 5
            : enemy.health <= 2 ? 4
              : committed && owner.stormRegentMove === "node-overcharge" && owner.stormRegentNodeId === enemy.id ? 3
                : committed && owner.stormRegentMove === "chain-strike" && isRelay ? 2
                  : phase === "entrance" || phase === "setup" ? 0 : 1;
          view.setTexture("storm-conductive-node-v1").setFrame(frame).setRotation(0).setScale(0.5);
        } else {
          const frame = enemy.health <= 0 ? 5 : enemy.health <= 2 ? 3 : 1 + (Math.floor(this.time.now / 180) % 2);
          view.setTexture("storm-node-v1").setFrame(frame).setRotation(0).setScale(0.5);
        }
        return;
      }
      case "scrap-skitterer": {
        const direction = enemy.scrapSkittererPhase === "approach"
          ? enemy.facingDirection
          : enemy.scrapSkittererDirection ?? enemy.facingDirection;
        const canonicalColumn = cardinalFacingColumn(enemy.position, {
        ...
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.scrapSkittererPhase ?? "approach";
        const row = phase === "rush-windup" ? 3 : phase === "rush" ? 4 : phase === "brake" ? 5 : 1 + ((Math.floor(this.time.now / 140) + enemy.id) % 2);
        view.setTexture("machine-scrap-skitterer-v1").setFrame(row * 4 + facingColumn).setRotation(0).setScale(0.62);
        return;
      }
      case "arc-warden": {
        const canonicalColumn = cardinalFacingColumn(enemy.position, {
      ...
      case "cyborg-reclaimer": {
        ...
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.reclaimerPhase ?? "seeking";
        const row = phase === "channel"
          ? 4
          : phase === "recovery" ? 6 : 1 + ((Math.floor(this.time.now / 190) + enemy.id) % 2);
        view.setTexture("machine-cyborg-reclaimer-v1").setFrame(row * 4 + facingColumn).setRotation(0).setScale(0.48);
        return;
      }
      case "foundry-fabricator": {
        const canonicalColumn = cardinalFacingColumn(enemy.position, { x: enemy.position.x + enemy.facingDirection.x, y: enemy.position.y + enemy.facingDirection.y });
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.foundryPhase ?? "positioning";
        const row = phase === "channel" ? 4 : phase === "recovery" ? 6 : 1 + ((Math.floor(this.time.now / 190) + enemy.id) % 2);
        view.setTexture("machine-foundry-fabricator-v1").setFrame(row * 4 + facingColumn).setRotation(0).setScale(0.48);
        return;
      }
      case "foundry-pad": {
        const owner = this.lastSnapshot.enemies.find((candidate) => candidate.id === enemy.foundryOwnerId);
        if (owner?.type === "assembly-prime") {
          const progress = owner.assemblyPrimeProgress ?? 0;
          const frame = progress >= 0.72 ? 3 : progress >= 0.34 ? 2 : progress > 0 ? 1 : 0;
          view.setTexture("assembly-prime-pad-v1").setFrame(frame).setRotation(0).setScale(0.5);
        } else {
          view.setTexture("machine-foundry-pad-v1").setFrame(1).setRotation(0).setScale(0.5);
        }
        return;
      }
      case "foundry-drone": {
        const canonicalColumn = cardinalFacingColumn(enemy.position, { x: enemy.position.x + enemy.facingDirection.x, y: enemy.position.y + enemy.facingDirection.y });
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const row = (enemy.foundryRemainingSeconds ?? 1) <= 0.5 ? 5 : 1 + ((Math.floor(this.time.now / 130) + enemy.id) % 2);
        view.setTexture("machine-foundry-drone-v1").setFrame(row * 4 + facingColumn).setRotation(0).setScale(0.48);
        return;
      }
      case "foundry-turret": {
        const canonicalColumn = cardinalFacingColumn(enemy.position, { x: enemy.position.x + enemy.facingDirection.x, y: enemy.position.y + enemy.facingDirection.y });
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.foundryTurretPhase ?? "tracking";
        const row = (enemy.foundryRemainingSeconds ?? 1) <= 0.5 ? 6 : phase === "warning" ? 2 : phase === "recovery" ? 4 : 1;
        view.setTexture("machine-foundry-turret-v1").setFrame(row * 4 + facingColumn).setRotation(0).setScale(0.5);
        return;
      }
      case "blast-mite": {
      ...
      case "synapse-herald": {
        ...
        const phase = enemy.synapseHeraldPhase ?? "entrance";
        const move = enemy.synapseHeraldMove;
        const row = phase === "entrance" ? 0
          : phase === "windup" ? move === "lunge-chain" ? 3 : move === "marked-zones" ? 5 : 6
            : phase === "action" ? move === "lunge-chain" ? 4 : move === "marked-zones" ? 5 : 6
              : phase === "recovery" ? 7
                : 1 + ((Math.floor(this.time.now / 180) + enemy.id) % 2);
        view.setTexture("synapse-herald-v1").setFrame(row * 4 + facingColumn).setRotation(0);
        return;
      }
      case "assembly-prime": {
        const target = { x: enemy.position.x + enemy.facingDirection.x, y: enemy.position.y + enemy.facingDirection.y };
        const canonicalColumn = cardinalFacingColumn(enemy.position, target);
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.assemblyPrimePhase ?? "entrance";
        const move = enemy.assemblyPrimeMove;
        const row = phase === "entrance" ? 0
          : phase === "setup" ? 1 + ((Math.floor(this.time.now / 190) + enemy.id) % 2)
            : phase === "windup" ? move === "rotating-lanes" ? 3
              : move === "fabrication" ? (enemy.assemblyPrimeProgress ?? 0) < 0.2 ? 5 : 6 : 7
              : phase === "action" ? move === "rotating-lanes" ? 4 : move === "fabrication" ? 6 : 7
                : 8;
        view.setTexture("assembly-prime-v1").setFrame(row * 4 + facingColumn).setRotation(0);
        return;
      }
      case "storm-regent": {
        const target = { x: enemy.position.x + enemy.facingDirection.x, y: enemy.position.y + enemy.facingDirection.y };
        const canonicalColumn = cardinalFacingColumn(enemy.position, target);
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.stormRegentPhase ?? "entrance";
        const move = enemy.stormRegentMove;
        const row = phase === "entrance" ? 0
          : phase === "setup" ? 1 + ((Math.floor(this.time.now / 220) + enemy.id) % 2)
            : phase === "windup" ? move === "chain-strike" ? 3 : move === "node-overcharge" ? 5 : 6
              : phase === "action" ? move === "chain-strike" ? 4 : move === "node-overcharge" ? 5 : 7
                : 8;
        view.setTexture("storm-regent-v1").setFrame(row * 4 + facingColumn).setRotation(0);
        return;
      }
      case "abomination-prime": {
        const target = { x: enemy.position.x + enemy.facingDirection.x, y: enemy.position.y + enemy.facingDirection.y };
        const canonicalColumn = cardinalFacingColumn(enemy.position, target);
        const facingColumn = [0, 3, 2, 1][canonicalColumn] ?? 0;
        const phase = enemy.abominationPrimePhase ?? "entrance";
        const move = enemy.abominationPrimeMove;
        const row = phase === "entrance" ? 0
          : phase === "setup" ? 1 + ((Math.floor(this.time.now / 240) + enemy.id) % 2)
            : phase === "windup" ? move === "ground-slam" ? 3 : move === "biomass-grab" ? 5 : 8
              : phase === "action" ? move === "ground-slam" ? 4 : move === "biomass-grab" ? 6 : 9
                : move === "biomass-grab" ? 7 : 10;
        view.setTexture("abomination-prime-v1").setFrame(row * 4 + facingColumn).setRotation(0);
        return;
      }
      default: {
  ...
  private syncEnemyProjectiles(projectiles: readonly EnemyProjectileSnapshot[]): void {
    ...
    for (const projectile of projectiles) {
      ...
      if (!view) {
        view = projectile.type === "prime-biomass" && this.useMarineArt
          ? this.add.sprite(0, 0, "abomination-prime-biomass-v1", 1).setDisplaySize(54, 38).setDepth(710)
          : projectile.type === "prime-biomass"
      ...
      if (view instanceof Phaser.GameObjects.Sprite) {
        ...
        if (projectile.type === "prime-biomass") {
          view.setFrame(1 + ((Math.floor(this.time.now / 120) + projectile.id) % 2)).setDisplaySize(54, 38);
        }
  ...
  private syncGroundHazards(hazards: readonly GroundHazardSnapshot[]): void {
    ...
    this.destroyMissing(this.hazardArtViews, liveIds);
    for (const hazard of hazards) {
      let view = this.hazardViews.get(hazard.id);
      if (!view) {
        view = hazard.type === "machine-wreck"
          ? this.useMarineArt
            ? this.add.sprite(0, 0, "machine-scrap-skitterer-v1", 28).setDepth(45)
      ...
      view.setPosition(hazard.position.x * PIXELS_PER_METRE, hazard.position.y * PIXELS_PER_METRE)
        .setDisplaySize(
          diameter * (hazard.type === "machine-wreck" ? 1 : 0.72 + lifetime * 0.28),
          diameter * (hazard.type === "machine-wreck" ? 0.55 : 0.62 * (0.72 + lifetime * 0.28)),
        )
      ...
      if (hazard.type === "prime-biomass" && this.useMarineArt) {
        let art = this.hazardArtViews.get(hazard.id);
        if (!art) {
          art = this.add.sprite(0, 0, "abomination-prime-biomass-v1", 4).setDepth(46).setOrigin(0.5, 0.72);
          this.hazardArtViews.set(hazard.id, art);
        }
        const frame = lifetime < 0.2 ? 6 : lifetime > 0.84 ? 4
          : 4 + ((Math.floor(this.time.now / 180) + hazard.id) % 2);
        art.setPosition(hazard.position.x * PIXELS_PER_METRE, hazard.position.y * PIXELS_PER_METRE)
          .setFrame(frame)
          .setDisplaySize(diameter * 0.72, diameter * 0.5)
          .setAlpha(0.7 + lifetime * 0.25);
      }
  ...
  private syncCorruptedMarineTelegraphs(enemies: readonly EnemySnapshot[]): void {
    ...
    for (const enemy of windups) {
      ...
      const edge = offscreenWarningPosition(enemy.position, {
      ...
      if (edge) {
        const direction = normalizeDirection(enemy.position.x - edge.x, enemy.position.y - edge.y);
        const perpendicular = { x: -direction.y, y: direction.x };
        const edgeX = edge.x * PIXELS_PER_METRE;
        const edgeY = edge.y * PIXELS_PER_METRE;
        view.fillStyle(0xffc45e, 0.95).fillTriangle(
          edgeX + direction.x * 13,
          edgeY + direction.y * 13,
          edgeX - direction.x * 8 + perpendicular.x * 9,
          edgeY - direction.y * 8 + perpendicular.y * 9,
          edgeX - direction.x * 8 - perpendicular.x * 9,
          edgeY - direction.y * 8 - perpendicular.y * 9,
        ).lineStyle(2, 0xff6b6b, 0.95).strokeCircle(edgeX, edgeY, 17);
      }
    }
  }
  ...
  private syncNestTelegraphs(enemies: readonly EnemySnapshot[]): void {
    const tracked = enemies.filter((enemy) => (
      (enemy.type === "nest-weaver" && enemy.nestWeaverPhase === "placement-windup")
      || enemy.type === "nest-pod"
    ));
    const liveIds = new Set(tracked.map((enemy) => enemy.id));
    this.destroyMissing(this.nestTelegraphs, liveIds);
    for (const enemy of tracked) {
      let view = this.nestTelegraphs.get(enemy.id);
      if (!view) {
        view = this.add.graphics().setDepth(64);
        this.nestTelegraphs.set(enemy.id, view);
      }
      view.clear();
      if (enemy.type === "nest-weaver" && enemy.nestWeaverTarget) {
        const fromX = enemy.position.x * PIXELS_PER_METRE;
        const fromY = enemy.position.y * PIXELS_PER_METRE;
        const targetX = enemy.nestWeaverTarget.x * PIXELS_PER_METRE;
        const targetY = enemy.nestWeaverTarget.y * PIXELS_PER_METRE;
        view.lineStyle(3, 0x7fe7d9, 0.82)
          .lineBetween(fromX, fromY, targetX, targetY)
          .fillStyle(0x8055aa, 0.2)
          .fillCircle(targetX, targetY, 22)
          .lineStyle(4, 0xb99be0, 0.95)
          .strokeCircle(targetX, targetY, 22);
        continue;
      }
      const x = enemy.position.x * PIXELS_PER_METRE;
      const y = enemy.position.y * PIXELS_PER_METRE;
      const remaining = Math.max(0, enemy.nestPodRemainingSeconds ?? 0);
      const pips = Math.max(1, Math.ceil(remaining));
      const pulse = 0.7 + Math.sin(this.time.now / 85) * 0.2;
      view.lineStyle(4, remaining <= 2 ? 0xffc45e : 0x7fe7d9, pulse)
        .strokeCircle(x, y, 30);
      for (let index = 0; index < pips; index += 1) {
        const angle = -Math.PI / 2 + index * Math.PI * 2 / 6;
        view.fillStyle(remaining <= 2 ? 0xff7a58 : 0xb99be0, 0.95)
          .fillCircle(x + Math.cos(angle) * 38, y + Math.sin(angle) * 38, 3.5);
      }
    }
  }

  private syncStormTelegraphs(enemies: readonly EnemySnapshot[]): void {
    const savants = enemies.filter((enemy) => (
      enemy.type === "storm-savant"
      && enemy.stormPhase !== "idle"
      && enemy.stormPhase !== "overload-recovery"
      && (enemy.stormSegments?.length ?? 0) > 0
    ));
    const liveIds = new Set(savants.map((enemy) => enemy.id));
    this.destroyMissing(this.stormTelegraphs, liveIds);
    for (const enemy of savants) {
      let view = this.stormTelegraphs.get(enemy.id);
      if (!view) {
        view = this.add.graphics().setDepth(63);
        this.stormTelegraphs.set(enemy.id, view);
      }
      view.clear();
      const discharge = enemy.stormPhase === "discharge";
      for (const segment of enemy.stormSegments ?? []) {
        ...
        const normalX = -dy / length * 5;
        const normalY = dx / length * 5;
        view.lineStyle(discharge ? 10 : 8, 0x081018, 0.9)
          .lineBetween(fromX, fromY, toX, toY)
          .lineStyle(discharge ? 6 : 4, discharge ? 0xf4ffff : 0x8de7ff, discharge ? 1 : 0.9)
          .lineBetween(fromX + normalX, fromY + normalY, toX + normalX, toY + normalY)
          .lineBetween(fromX - normalX, fromY - normalY, toX - normalX, toY - normalY);
        for (const fraction of [0.25, 0.5, 0.75]) {
          const rungX = fromX + dx * fraction;
          const rungY = fromY + dy * fraction;
          view.lineBetween(
            rungX - normalX,
            rungY - normalY,
            rungX + normalX,
        ...
        if (segment.blockedByObstacleId) {
          view.lineStyle(4, 0xffe47a, 0.95).strokeRect(toX - 9, toY - 9, 18, 18);
        } else {
          view.lineStyle(3, 0xffe47a, 0.9).strokeCircle(toX, toY, 18);
        }
      }
    }
  }

  private syncScrapSkittererTelegraphs(enemies: readonly EnemySnapshot[]): void {
    const warning = enemies.filter((enemy) => (
      enemy.type === "scrap-skitterer"
      && enemy.scrapSkittererPhase === "rush-windup"
      && enemy.scrapSkittererDirection
    ));
    const liveIds = new Set(warning.map((enemy) => enemy.id));
    this.destroyMissing(this.scrapSkittererTelegraphs, liveIds);
    for (const enemy of warning) {
      let view = this.scrapSkittererTelegraphs.get(enemy.id);
      if (!view) {
        view = this.add.graphics().setDepth(62);
        this.scrapSkittererTelegraphs.set(enemy.id, view);
      }
      const direction = enemy.scrapSkittererDirection!;
      const x = enemy.position.x * PIXELS_PER_METRE;
      const y = enemy.position.y * PIXELS_PER_METRE;
      const endX = x + direction.x * 3.2 * PIXELS_PER_METRE;
      const endY = y + direction.y * 3.2 * PIXELS_PER_METRE;
      const normal = { x: -direction.y * 7, y: direction.x * 7 };
      view.clear()
        .lineStyle(8, 0x081018, 0.85)
        .lineBetween(x, y, endX, endY)
        .lineStyle(4, 0xffd36b, 0.95)
        .lineBetween(x + normal.x, y + normal.y, endX + normal.x, endY + normal.y)
        .lineBetween(x - normal.x, y - normal.y, endX - normal.x, endY - normal.y)
        .fillStyle(0xffd36b, 0.95)
        .fillTriangle(
          endX + direction.x * 12,
          endY + direction.y * 12,
          endX - direction.x * 8 + normal.x,
          endY - direction.y * 8 + normal.y,
          endX - direction.x * 8 - normal.x,
          endY - direction.y * 8 - normal.y,
        );
    }
  ...
  private syncArcWardenTelegraphs(enemies: readonly EnemySnapshot[]): void {
    const wardens = enemies.filter((enemy) => (
      enemy.type === "arc-warden"
      && (enemy.arcWardenPhase === "charge" || enemy.arcWardenPhase === "discharge")
      && enemy.arcWardenLane
    ));
    const liveIds = new Set(wardens.map((enemy) => enemy.id));
    this.destroyMissing(this.arcWardenTelegraphs, liveIds);
    for (const enemy of wardens) {
      let view = this.arcWardenTelegraphs.get(enemy.id);
      if (!view) {
        view = this.add.graphics().setDepth(63);
      ...
      if (!discharge) {
        for (const fraction of [0.2, 0.4, 0.6, 0.8]) {
          const x = fromX + dx * fraction;
          const y = fromY + dy * fraction;
          view.lineBetween(x - normal.x, y - normal.y, x + normal.x, y + normal.y);
        }
      }
      if (lane.blockedByObstacleId) {
        view.lineStyle(4, 0xffe47a, 0.98).strokeRect(toX - 9, toY - 9, 18, 18);
      } else {
        view.lineStyle(4, 0xffe47a, 0.98)
          .strokePoints([
  ...
  private syncReclaimerLinks(enemies: readonly EnemySnapshot[]): void {
    const reclaimers = enemies.filter((enemy) => (
      enemy.type === "cyborg-reclaimer"
      && enemy.reclaimerPhase === "channel"
      && enemy.reclaimerTargetId !== undefined
      && enemies.some((target) => target.id === enemy.reclaimerTargetId)
    ));
    const liveIds = new Set(reclaimers.map((enemy) => enemy.id));
    this.destroyMissing(this.reclaimerLinks, liveIds);
    for (const enemy of reclaimers) {
      const target = enemies.find((candidate) => candidate.id === enemy.reclaimerTargetId)!;
      let view = this.reclaimerLinks.get(enemy.id);
      if (!view) {
        view = this.add.graphics().setDepth(64);
        this.reclaimerLinks.set(enemy.id, view);
      }
      const fromX = enemy.position.x * PIXELS_PER_METRE;
      const fromY = enemy.position.y * PIXELS_PER_METRE;
      const toX = target.position.x * PIXELS_PER_METRE;
      const toY = target.position.y * PIXELS_PER_METRE;
      const dx = toX - fromX;
      const dy = toY - fromY;
      const pulse = 0.78 + Math.sin(this.time.now / 85) * 0.17;
      view.clear()
        .lineStyle(9, 0x081018, 0.9)
        .lineBetween(fromX, fromY, toX, toY)
        .lineStyle(4, 0x8ff0ae, pulse)
        .lineBetween(fromX, fromY, toX, toY);
      for (const fraction of [0.2, 0.4, 0.6, 0.8]) {
        view.fillStyle(0xffe47a, 0.95)
          .fillCircle(fromX + dx * fraction, fromY + dy * fraction, 3.5);
      }
      view.lineStyle(3, 0x8ff0ae, 0.95)
        .strokeCircle(toX, toY, (target.radiusMetres * PIXELS_PER_METRE) + 8)
        .lineBetween(toX - 8, toY, toX + 8, toY)
        .lineBetween(toX, toY - 8, toX, toY + 8);
    }
  }

  private syncFoundryTurretTelegraphs(enemies: readonly EnemySnapshot[]): void {
    const warningTurrets = enemies.filter((enemy) => (
      enemy.type === "foundry-turret"
      && enemy.foundryTurretPhase === "warning"
      && enemy.foundryTurretTarget
    ));
    const liveIds = new Set(warningTurrets.map((enemy) => enemy.id));
    this.destroyMissing(this.foundryTurretTelegraphs, liveIds);
    for (const enemy of warningTurrets) {
      let view = this.foundryTurretTelegraphs.get(enemy.id);
      if (!view) {
        view = this.add.graphics().setDepth(63);
        this.foundryTurretTelegraphs.set(enemy.id, view);
      }
      const target = enemy.foundryTurretTarget!;
      const fromX = enemy.position.x * PIXELS_PER_METRE;
      const fromY = enemy.position.y * PIXELS_PER_METRE;
      ...
      const dy = toY - fromY;
      const length = Math.max(1, Math.hypot(dx, dy));
      const normal = { x: -dy / length * 3.5, y: dx / length * 3.5 };
      view.clear()
        .lineStyle(10, 0x081018, 0.9)
        .lineBetween(fromX, fromY, toX, toY)
        .lineStyle(3, 0xffb85c, 0.96)
        .lineBetween(fromX + normal.x, fromY + normal.y, toX + normal.x, toY + normal.y)
        .lineBetween(fromX - normal.x, fromY - normal.y, toX - normal.x, toY - normal.y)
        .lineStyle(3, 0xffe47a, 0.98)
        .strokeCircle(toX, toY, 12);
      for (const fraction of [0.25, 0.5, 0.75]) {
        const x = fromX + dx * fraction;
        const y = fromY + dy * fraction;
        view.lineBetween(x - normal.x, y - normal.y, x + normal.x, y + normal.y);
  ...
  private syncMiniBossTelegraphs(enemies: readonly EnemySnapshot[]): void {
    ...
    for (const boss of bosses) {
      ...
      if (boss.miniBossKind === "brood-warden") {
      ...
      } else if (boss.miniBossKind === "synapse-herald") {
        const move = boss.synapseHeraldMove;
        const targets = boss.synapseHeraldTargets ?? [];
        if (move === "lunge-chain" && (boss.synapseHeraldPhase === "windup" || boss.synapseHeraldPhase === "action")) {
          let fromX = x;
          let fromY = y;
          for (const [index, target] of targets.entries()) {
            const toX = target.x * PIXELS_PER_METRE;
            const toY = target.y * PIXELS_PER_METRE;
            view.lineStyle(9, 0x081018, 0.88).lineBetween(fromX, fromY, toX, toY)
              .lineStyle(3, index % 2 === 0 ? 0xe5a4ff : 0xffe47a, 0.9)
              .lineBetween(fromX, fromY, toX, toY)
              .strokeCircle(toX, toY, 11 + index * 2);
            fromX = toX;
            fromY = toY;
          }
        } else if (move === "marked-zones" && (boss.synapseHeraldPhase === "windup" || boss.synapseHeraldPhase === "action")) {
          for (const target of targets) {
            const toX = target.x * PIXELS_PER_METRE;
            const toY = target.y * PIXELS_PER_METRE;
            view.lineStyle(8, 0x081018, 0.88).strokeCircle(toX, toY, 1.35 * PIXELS_PER_METRE)
              .lineStyle(4, 0xff8fcf, 0.9).strokeCircle(toX, toY, 1.35 * PIXELS_PER_METRE)
              .lineBetween(toX - 12, toY, toX + 12, toY)
              .lineBetween(toX, toY - 12, toX, toY + 12);
          }
        } else if (move === "synapse-link" && boss.synapseHeraldLinkTargetId !== undefined) {
          const target = enemies.find((candidate) => candidate.id === boss.synapseHeraldLinkTargetId);
          if (target) {
            const toX = target.position.x * PIXELS_PER_METRE;
            const toY = target.position.y * PIXELS_PER_METRE;
        ...
        if (boss.assemblyPrimeMove === "rotating-lanes") {
          for (const [index, lane] of (boss.assemblyPrimeLanes ?? []).entries()) {
            const toX = (lane.origin.x + lane.direction.x * 11) * PIXELS_PER_METRE;
            const toY = (lane.origin.y + lane.direction.y * 11) * PIXELS_PER_METRE;
            view.lineStyle(10, 0x081018, 0.9).lineBetween(x, y, toX, toY)
              .lineStyle(3, index % 2 === 0 ? 0xffb84d : 0x8de7ff, 0.95).lineBetween(x, y, toX, toY);
            for (let rung = 1; rung <= 3; rung += 1) {
              view.fillStyle(0xffffff, 0.9).fillCircle(x + (toX - x) * rung / 4, y + (toY - y) * rung / 4, 4 + index);
            }
          }
        } else if (boss.assemblyPrimeMove === "fabrication" && boss.assemblyPrimeTarget) {
          const tx = boss.assemblyPrimeTarget.x * PIXELS_PER_METRE;
          const ty = boss.assemblyPrimeTarget.y * PIXELS_PER_METRE;
          view.lineStyle(9, 0x081018, 0.9).strokeCircle(tx, ty, 24)
            .lineStyle(4, 0xffb84d, 0.95).strokeCircle(tx, ty, 24).lineBetween(x, y, tx, ty);
        } else if (boss.assemblyPrimeMove === "drone-recall" && boss.assemblyPrimeRecallTargetId !== undefined) {
          const target = enemies.find((candidate) => candidate.id === boss.assemblyPrimeRecallTargetId);
          if (target) {
            const tx = target.position.x * PIXELS_PER_METRE;
            const ty = target.position.y * PIXELS_PER_METRE;
            view.lineStyle(10, 0x081018, 0.9).lineBetween(x, y, tx, ty)
              .lineStyle(4, 0x8de7ff, 0.95).lineBetween(x, y, tx, ty).strokeCircle(tx, ty, 18);
          }
        }
