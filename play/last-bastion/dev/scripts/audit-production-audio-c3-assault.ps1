...
$runtime = Join-Path $devRoot "src/game/audio/runtime/batch-c3-assault"
$expectedStems = @("assault-damage", "assault-death", "assault-evade")
...
$results = foreach ($stem in $expectedStems) {
  foreach ($extension in @("ogg", "mp3")) {
    ...
    try {
      ...
      # ErrorActionPreference is Stop. FFmpeg reports normal probe data there.
...
$reportPath = Join-Path $projectRoot "audio/production/batch-c3-assault/c3-assault-audio-audit.json"
$results | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $reportPath -Encoding utf8
Write-Output "C3 Assault audio audit complete: $($results.Count) derivatives."
