...
$masters = Join-Path $projectRoot "audio/production/batch-c3-scout/masters"
$runtime = Join-Path $projectRoot "dev/src/game/audio/runtime/batch-c3-scout"
...
foreach ($wav in Get-ChildItem -LiteralPath $masters -Filter "*.wav" -File | Sort-Object Name) {
  ...
  Write-Output "ENCODED batch-c3-scout/$stem"
}

Write-Output "C3 Scout encoding complete. Run npm.cmd run audio:audit:c3-scout -- -FfmpegPath <path>."
