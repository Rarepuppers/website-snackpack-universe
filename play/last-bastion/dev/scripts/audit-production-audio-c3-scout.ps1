...
$runtime = Join-Path $devRoot "src/game/audio/runtime/batch-c3-scout"
$expectedStems = @("scout-damage", "scout-death", "scout-evade")
...
$reportPath = Join-Path $projectRoot "audio/production/batch-c3-scout/c3-scout-audio-audit.json"
$results | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $reportPath -Encoding utf8
Write-Output "C3 Scout audio audit complete: $($results.Count) derivatives."
