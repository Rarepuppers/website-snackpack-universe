...
$masters = Join-Path $projectRoot "audio/production/batch-c3-assault/masters"
$runtime = Join-Path $projectRoot "dev/src/game/audio/runtime/batch-c3-assault"
...
foreach ($wav in Get-ChildItem -LiteralPath $masters -Filter "*.wav" -File | Sort-Object Name) {
  ...
  Write-Output "ENCODED batch-c3-assault/$stem"
}

Write-Output "C3 Assault encoding complete. Run npm.cmd run audio:audit:c3-assault -- -FfmpegPath <path>."
