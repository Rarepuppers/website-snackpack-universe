...
**Damage numbers (confirmed 18 July 2026; density revised 20 July):** small floating numerals at the impact point, damage-type coloured — **standard ivory/white for normal/physical, red for fire, blue for frozen/cryo, teal for lightning/shock, green for poison/toxic** — crits/weak points larger, healing ticks small green with a `+`. Pooled like existing effects, capped at **24 concurrent labels** with hits merged per enemy per 100 ms. At the cap, ordinary enemy labels recycle first; fresh ordinary hits may be omitted rather than erase incoming-player or healing feedback. Setting: Damage numbers On/Off.
...
- **Corrupted Human promotion remains gated.** Tasks 58–59 prove the eight-unit Survivor pulse and the Corrupted Marine's locked slow-knife lifecycle. Do not add either to director compositions until Task 62 completes the Abomination and mixed-family/off-screen-warning pass; when promoted, spend Survivors as one readable pulse and never layer two survivor packs simultaneously.
