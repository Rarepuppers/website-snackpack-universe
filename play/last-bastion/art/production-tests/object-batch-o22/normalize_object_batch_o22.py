"""Build deterministic Object Batch O22 civic, agricultural, and utility atlases."""
...
PARTS = {"civic-community-v1": "CIVIC / COMMUNITY", "agricultural-hydroponic-v1": "AGRICULTURAL / HYDROPONIC", "utility-room-v1": "UTILITY ROOM"}
...
def main() -> None:
    ...
    draw.text((24, 18), "OBJECT BATCH O22 / CIVIC + AGRICULTURAL + UTILITY / 4K PREFLIGHT", fill=(231, 240, 246, 255))
    y = 70
    for name, label in PARTS.items():
        ...
        y += 450
    draw.text((24, 2070), "Civic, agricultural, and utility candidates; no interaction, collision, hazards, targets, telegraphs, or gameplay values baked in.", fill=(184, 202, 216, 255))
    canvas.convert("RGB").save(ROOT / "object-batch-o22-contact-sheet.png", optimize=True)
    runtime_dir = ROOT.parents[2] / "game-assets"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    for name in PARTS:
        for size in (256, 128):
            copy2(ROOT / f"{name}-{size}.png", runtime_dir / f"{name}-{size}.png")
    print("Wrote Object Batch O22 masters and runtime derivatives")
