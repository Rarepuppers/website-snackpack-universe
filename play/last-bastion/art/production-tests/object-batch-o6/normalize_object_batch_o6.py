"""Build deterministic Object Batch O6 structural accent atlases."""
...
PARTS = {
    "ventilation-ducts-v1": "VENTILATION / DUCTS",
    "storage-logistics-v1": "STORAGE / LOGISTICS",
    "power-infrastructure-v1": "POWER INFRASTRUCTURE",
}
...
def main() -> None:
    ...
    draw.text((24, 18), "OBJECT BATCH O6 / STRUCTURAL ACCENTS / 4K PREFLIGHT", fill=(231, 240, 246, 255))
    y = 70
    for name, label in PARTS.items():
        ...
        y += 450
    draw.text((24, 2070), "Structural accent candidates; no interaction, collision, targets, telegraphs, or gameplay values baked in.", fill=(184, 202, 216, 255))
    canvas.convert("RGB").save(ROOT / "object-batch-o6-contact-sheet.png", optimize=True)
    runtime_dir = ROOT.parents[2] / "game-assets"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    for name in PARTS:
        for size in (256, 128):
            copy2(ROOT / f"{name}-{size}.png", runtime_dir / f"{name}-{size}.png")
    print("Wrote Object Batch O6 masters and runtime derivatives")
