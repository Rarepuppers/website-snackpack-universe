"""Build deterministic Object Batch O18 habitation and communications atlases."""
...
PARTS = {"habitation-v1": "HABITATION", "recreation-common-v1": "RECREATION / COMMON", "communications-v1": "COMMUNICATIONS"}
...
def main() -> None:
    ...
    draw.text((24, 18), "OBJECT BATCH O18 / HABITATION + RECREATION + COMMS / 4K PREFLIGHT", fill=(231, 240, 246, 255))
    y = 70
    for name, label in PARTS.items():
        ...
        y += 450
    draw.text((24, 2070), "Habitation and communications candidates; no interaction, collision, hazards, targets, telegraphs, or gameplay values baked in.", fill=(184, 202, 216, 255))
    canvas.convert("RGB").save(ROOT / "object-batch-o18-contact-sheet.png", optimize=True)
    runtime_dir = ROOT.parents[2] / "game-assets"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    for name in PARTS:
        for size in (256, 128):
            copy2(ROOT / f"{name}-{size}.png", runtime_dir / f"{name}-{size}.png")
    print("Wrote Object Batch O18 masters and runtime derivatives")
