"""Build deterministic Object Batch O15 common-room and bay atlases."""
...
PARTS = {"crew-common-v1": "CREW / COMMON ROOM", "vehicle-bay-v1": "VEHICLE BAY", "recycling-waste-v1": "RECYCLING / WASTE"}
...
def restore(name: str) -> None:
    ...
    # Deterministically clear single-pixel chroma fringe at atlas corners.
...
def main() -> None:
    ...
    draw.text((24, 18), "OBJECT BATCH O15 / COMMON + VEHICLE BAY / 4K PREFLIGHT", fill=(231, 240, 246, 255))
    y = 70
    for name, label in PARTS.items():
        ...
        y += 450
    draw.text((24, 2070), "Common-room and bay candidates; no interaction, collision, targets, telegraphs, or gameplay values baked in.", fill=(184, 202, 216, 255))
    canvas.convert("RGB").save(ROOT / "object-batch-o15-contact-sheet.png", optimize=True)
    runtime_dir = ROOT.parents[2] / "game-assets"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    for name in PARTS:
        for size in (256, 128):
            copy2(ROOT / f"{name}-{size}.png", runtime_dir / f"{name}-{size}.png")
    print("Wrote Object Batch O15 masters and runtime derivatives")
