"""Build deterministic Object Batch O5 room-dressing atlases."""
...
PARTS = {
    "maintenance-dressing-v1": "MAINTENANCE DRESSING",
    "medical-cryogenic-dressing-v1": "MEDICAL / CRYOGENIC DRESSING",
    "hive-dressing-v1": "HIVE DRESSING",
}
...
def main() -> None:
    ...
    draw.text((24, 18), "OBJECT BATCH O5 / ROOM DRESSING / 4K PREFLIGHT", fill=(231, 240, 246, 255))
    y = 70
    for name, label in PARTS.items():
        ...
        y += 450
    draw.text((24, 2070), "Room-dressing candidates; no interaction, collision, targets, telegraphs, or gameplay values baked in.", fill=(184, 202, 216, 255))
    canvas.convert("RGB").save(ROOT / "object-batch-o5-contact-sheet.png", optimize=True)
    runtime_dir = ROOT.parents[2] / "game-assets"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    for name in PARTS:
        for size in (256, 128):
            copy2(ROOT / f"{name}-{size}.png", runtime_dir / f"{name}-{size}.png")
    print("Wrote Object Batch O5 masters and runtime derivatives")
