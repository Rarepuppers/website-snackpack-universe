"""Build deterministic Object Batch O19 command, navigation, and archive atlases."""
...
PARTS = {"command-center-v1": "COMMAND CENTER", "navigation-observation-v1": "NAVIGATION / OBSERVATION", "archive-records-v1": "ARCHIVE / RECORDS"}
...
def main() -> None:
    ...
    draw.text((24, 18), "OBJECT BATCH O19 / COMMAND + NAVIGATION + ARCHIVE / 4K PREFLIGHT", fill=(231, 240, 246, 255))
    y = 70
    for name, label in PARTS.items():
        ...
        y += 450
    draw.text((24, 2070), "Command, navigation, and archive candidates; no interaction, collision, hazards, targets, telegraphs, or gameplay values baked in.", fill=(184, 202, 216, 255))
    canvas.convert("RGB").save(ROOT / "object-batch-o19-contact-sheet.png", optimize=True)
    runtime_dir = ROOT.parents[2] / "game-assets"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    for name in PARTS:
        for size in (256, 128):
            copy2(ROOT / f"{name}-{size}.png", runtime_dir / f"{name}-{size}.png")
    print("Wrote Object Batch O19 masters and runtime derivatives")
