...
PARTS = {"construction-staging-v1": "CONSTRUCTION / STAGING", "geology-survey-v1": "GEOLOGY / SURVEY", "quiet-recreation-v1": "QUIET / RECREATION"}
...
def normalize(src, size):
    cw, ch = src.width // 2, src.height // 2; out = Image.new("RGBA", (size * 2, size * 2), (0, 0, 0, 0))
    for r in range(2):
        for c in range(2): out.alpha_composite(src.crop((c * cw, r * ch, (c + 1) * cw, (r + 1) * ch)).resize((size, size), Image.Resampling.LANCZOS), (c * size, r * size))
    return out
...
def contact_sheet(masters):
    ...
    draw.text((48, 36), "OBJECT BATCH O30 / CONSTRUCTION STAGING + GEOLOGY SURVEY + QUIET RECREATION / 4K PREFLIGHT", fill=(240, 244, 248), font=font)
    y = 110
    ...
    draw.text((48, 3440), "Construction-staging, geology-survey, and quiet-recreation candidates; no interaction, collision, hazards, targets, telegraphs, or gameplay values baked in.", fill=(178, 186, 196), font=small)
    return sheet
...
def main():
    ...
    contact_sheet(masters).save(ROOT / "object-batch-o30-contact-sheet.jpg", quality=95, optimize=True)
