"""Build deterministic Object Batch O7 composition accents."""
...
PARTS = {
    "debris-rubble-v1": "DEBRIS / RUBBLE",
    "floor-trims-v1": "FLOOR EDGE TRIMS",
    "light-fixtures-v1": "LIGHT FIXTURES",
}
...
def main() -> None:
    ...
    draw.text((24, 18), "OBJECT BATCH O7 / COMPOSITION ACCENTS / 4K PREFLIGHT", fill=(231, 240, 246, 255))
    y = 70
    ...
    canvas.convert("RGB").save(ROOT / "object-batch-o7-contact-sheet.png", optimize=True)
    runtime_dir = ROOT.parents[2] / "game-assets"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    for name in PARTS:
        for size in (256, 128):
            copy2(ROOT / f"{name}-{size}.png", runtime_dir / f"{name}-{size}.png")
    print("Wrote Object Batch O7 masters and runtime derivatives")
