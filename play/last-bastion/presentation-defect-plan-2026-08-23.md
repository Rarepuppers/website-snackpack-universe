...
>
> No longer plan-only. Closed since: **LB-01** (projectiles render), **LB-03** (crates had a
> cue and no picture — invisible, not silent), **LB-05** (title sits inside its plate),
> **LB-06** (perk rail now derives its shape from the panel), **LB-08** (arm-then-deploy),
> **LB-09** (whole-device-pixel text), **LB-10** (evidence folder), **LB-11** (measure-then-size
> stat cards), **LB-12** (dead glyph table removed).
>
> **LB-02 closed** the same day: power-ups now carry an authored name and a one-line
> description drawn from the simulation's own constants, shown on pickup. Still open:
> **LB-04** (helmet offset — art, Codex) and **LB-07** (map node iconography — art, Codex).
> **Every code-owned presentation defect in this register is now closed.**
>
> §5.4's overflow audit landed as `shell/shellTextLayout.test.ts` and immediately found a
> defect nobody had reported: at a fixed 12px, **four of the five hero dossiers ran past the
> PERK heading**, the locked Tactician by 51px. Since only the Marine starts unlocked, that
> was the common case, not an edge one. Fixed by putting the dossier through the same
> measure-then-size rule.
>
> §5.3 combat-event presentation coverage is **complete**: 21 events reached the player through
> no surface at all; the count is now zero and guarded at zero. §5.1 measure-then-size shipped as
> `ui/MeasuredText.ts` and §5.4 as the overflow audit above. **§5.2 one text factory is the
> only structural item still open — and on 12 September it was investigated and
> deliberately not done.**
>
> The reason it looked urgent was a grep: none of the six non-combat scenes call
> `uiTextResolution()`, only combat does, which suggested LB-09's fix had never reached the
> menus and that every shell screen was rendering upscaled text at 1080p and 4K. **The pixels
> do not support that.** A tight 4K crop of the title's "PRESS ENTER" shows clean ~1px
> antialiasing, not the 4px smear a 1x texture drawn at 4x would produce. The presentation
> path resizes the backing store to physical resolution and the text comes out crisp without
> the explicit call.
>
> So §5.2's remaining value is de-duplicating six near-identical private `text()` helpers:
> real, but a maintainability refactor with regression risk and no player-visible benefit,
> which is exactly the kind of work this plan puts behind everything else. Left open
> deliberately. Recorded here so the grep does not raise the alarm again.
>
> Evidence: `playtest-evidence/2026-09-11-screens-auto/`, generated by
> `npm run screens:last-bastion` (QA-04b). Two defects were found by those pictures that no
> source review had caught: a threat arrow sitting on the pause hint, and LB-06's real extent.
