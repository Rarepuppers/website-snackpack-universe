...
- **Encounter mapping (Tasks 39 and 48 live):** Combat nodes run three or four depth-budgeted waves; Elite nodes run two lead waves plus a seeded kill-owned elite/cache wave; Mini-boss nodes run one arena wave plus the authored seeded fight; Supply Depot and Weapon Cache reuse the existing full-screen decisions; the Boss node runs the Bastion Eater. Internal waves remain inside the encounter and only final victory commits health, shield, level, XP, Scrap, weapon tiers, and upgrades back to the map save. Quick Drop retains its independent ten-wave script.
