...
### Run summary and Records (Task 50 live)

Every victory or defeat now resolves through a persisted debrief rather than returning directly to the title. It reports the run mode and outcome, hero, final level, wave or nodes reached, kills, gross Scrap earned, banked Scrap, damage split by weapon, final weapon tiers, upgrade levels, and any perks whose milestone was crossed during that run. Expedition metrics accumulate across encounter hand-offs; Quick Drop records its single combat session. Status-effect ticks are not mislabelled as direct weapon-hit damage.

The Records card is a real main-menu destination backed by the same schema-v4 save data. It shows runs finished, victories, best wave/column, best expedition nodes, lifetime nodes cleared, enemies defeated, total damage, and Scrap earned. The debrief and ledger are code-native HUD surfaces, so they remain sharp at 1080p and 4K without consuming a raster batch.
...
- **Node types and budget per map:** Combat (majority), Elite ×2 (guaranteed cache), Mini-boss ×1–2 (drawn from the seeded pool), Supply Depot ×2 (the existing heal/armoury/shield decision as a safe node), Weapon Cache ×2 (the existing chest as a node), and the Boss terminus. The Scrap Shop is deliberately **not** a node type: every route instead receives post-encounter shop stops after zero-based columns 3 and 5, fronted by varied shop keepers and, in the future, keeper specialties (see Task 45 in `last-bastion-model.md`). Shrine and Event nodes are reserved future types and render as design placeholders until their systems exist.
...
- **Encounter mapping (Tasks 39, 48, and 49 live):** Combat nodes run three or four depth-budgeted waves; Elite nodes run two lead waves plus a seeded kill-owned elite/cache wave; Mini-boss nodes run one arena wave plus the authored seeded fight; Supply Depot and Weapon Cache reuse the existing full-screen decisions and pay 10 Scrap; the Boss node runs the Bastion Eater. Internal waves remain inside the encounter and only final victory commits health, shield, level, XP, Scrap, weapon tiers, and upgrades back to the map save. Quick Drop retains its independent ten-wave script.
