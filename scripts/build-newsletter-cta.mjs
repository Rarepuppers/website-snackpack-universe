  ...
  <section>
    <div class="shell">
      <div class="signup-card">
        ...
        <p class="section-copy">New SnackPack apps, updates to ${appName}, and the occasional behind-the-scenes note. No spam, and you can unsubscribe in one click.</p>
...
for (const slug of fs.readdirSync(appsDir).sort()) {
  ...
  if (original.includes(OPEN)) {
    ...
    // Last thing before </main>: the reader has finished the page, and the
    // onward internal links above it still get their chance first.
    const anchor = original.lastIndexOf("</main>");
    if (anchor < 0) {
      stale.push(`apps/${slug}/index.html: no </main> to anchor to`);
      continue;
    }
  ...
  if (updated !== original) {
    if (checkOnly) stale.push(`apps/${slug}/index.html: newsletter block missing or out of date`);
    else {
